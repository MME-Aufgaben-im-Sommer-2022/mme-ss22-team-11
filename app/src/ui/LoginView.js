...
function getUserData(el) {
    ...
    let email = el.querySelector(".login-email-input").value;
    let password = el.querySelector(".login-password-input").value;
...
class LoginView extends Observable {
    ...
    initializeLoginView() {
        this.el.querySelector(".login-link").addEventListener("click", (event) => {
            toggleSignUpLogin(this.el);
        })
        
        this.el.querySelector(".signup-button").addEventListener("click", (event) => {
