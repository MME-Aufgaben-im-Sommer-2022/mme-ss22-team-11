...
class CocktailListManager extends Observable {
  ...
  constructor() {
    ...
    this.ingredientList.addEventListener("INGREDIENTS_READY", (event) => this
  ...
  addCocktailFromJSON(id, name, recipe, image, description, steps, author) {
    ...
    let newCocktail = new Cocktail(id, name, recipe, image, [], [],
      description, steps, author);
      ...
      .then((json) => {
        for (let i in json) { //eslint-disable-line
          ...
          this.addCocktailFromJSON(i, data.name, recipe, data.img, data
            .description, data.steps, data.author);
        }
  ...
  async createImage(data) {
    return await this.appwrite.createFile(data);
  }
  ...
  getCocktailsFromIngredients(ingredients, withDeco) {
    let returnList = [];
    this.allCocktails.forEach(cocktail => {
      if (cocktail.checkIfCocktailHasOnlyTheseIngredients(ingredients,
          withDeco)) {
        returnList.push(cocktail);
      }
    });
